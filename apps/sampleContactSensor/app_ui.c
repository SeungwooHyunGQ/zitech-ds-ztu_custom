...
void app_sensor_handler(void)
{
    if (!zb_isDeviceJoinedNwk()) {
        return;
    }

    u8 curAlarm  = drv_gpio_read(SENSOR_ALARM_PIN) ? 1 : 0;
    u8 curTamper = drv_gpio_read(SENSOR_TAMPER_PIN) ? 1 : 0;

    if (curAlarm != g_sensorAppCtx.alarmState || curTamper != g_sensorAppCtx.tamperState) {
        g_sensorAppCtx.alarmState  = curAlarm;
        g_sensorAppCtx.tamperState = curTamper;

        u16 zoneStatus = 0;
        if (curAlarm)  zoneStatus |= ZONE_STATUS_BIT_ALARM1;
        if (curTamper) zoneStatus |= ZONE_STATUS_BIT_TAMPER;

        g_zcl_iasZoneAttrs.zoneStatus = zoneStatus;
        ...
        noti.zoneStatus = zoneStatus;
        noti.extStatus  = 0;
        noti.zoneId     = ZCL_ZONE_ID_INVALID;
        noti.delay      = 0;

        zcl_iasZone_statusChangeNotificationCmd(SAMPLE_SENSOR_ENDPOINT, &dstEpInfo, TRUE, &noti);
    }
}
